...
installedVersion=$(/usr/local/bin/php -v | grep -ioP '(?<=PHP )\d\.\d{1,2}\.\d{1,3}') || exit 1
currentVersion=$(curl 'http://api.sinosky.org/version.php?pro=php') || exit 1
...
cd /usr/local/src/php-extension/xcache-3.1.0 && /usr/local/php/bin/phpize && ./configure --with-php-config=/usr/local/php/bin/php-config --enable-xcache --enable-xcache-coverager --enable-xcache-optimizer && make && make install
